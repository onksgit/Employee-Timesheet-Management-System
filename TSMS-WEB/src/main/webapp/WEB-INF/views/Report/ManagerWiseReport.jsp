...
<div class="row">
	...
	<div class="col-lg-10">
		<div class="col-lg-12" align="left">
			<h3 align="center" class="title">Manager Wise Report</h3>
		</div>
	</div>
...
<div class="well well-lg col-lg-6">
	<form:form name="managerWiseReport" method="POST" class="form-horizontal" modelAttribute="reportPojo" role="form"
		...
		<div class="form-group inputpd">
			...
			<div class="col-sm-6">
				<form:select path="managername" id="managerName" class="form-control">
					<option value="0" selected>-Select-</option>
					<c:forEach var="managerList" items="${managerList}">
						<c:choose>
							<c:when test="${managerList.primaryId.toString()==managerid}">
								<option value="${managerList.primaryId}" selected>${managerList.firstName} ${managerList.lastName}</option>
							</c:when>
							<c:otherwise>
								<option value="${managerList.primaryId}">${managerList.firstName} ${managerList.lastName}</option>
							</c:otherwise>
						</c:choose>
					</c:forEach>
				</form:select>
	...
	<c:when test="${showData eq true}">
		...
		<div class="table-responsive">
			<table class="table table-hover" id="managerReport">
				...
				<tbody>
					<c:choose>
						<c:when test="${not empty listManagerWiseReport}">
							<c:forEach var="listManagerWiseReport" items="${listManagerWiseReport}">
								<tr>
									<td>${listManagerWiseReport.managerName}</td>
									<td>${listManagerWiseReport.employeeName}</td>
									<td>${listManagerWiseReport.timesheetWork}</td>
									<td>${listManagerWiseReport.projectName}</td>
									<td>${listManagerWiseReport.approvalStatus}</td>
								</tr>
							</c:forEach>
						</c:when>
						<c:otherwise>
